...
#                            for j in range(len(sessions[0]['states']))])
        ...
        plt.subplot(233)
        plt.plot(self.noise_thresholds, 'g', label='noise_threshold')
        plt.legend()
        plt.grid()

        plt.subplot(234)
        plt.plot(env.sigma_array, label=r'$sigma$')
        plt.legend()
        plt.grid()

        plt.subplot(235)
        plt.plot(env.fx_array, label='Fx')
        plt.legend()
        plt.grid()

        plt.subplot(236)
        plt.plot(env.fz_array, label='Fz')
        plt.legend()
        plt.grid()
